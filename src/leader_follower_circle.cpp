...
const double u_bound = 0.2;//1.5
const double l_bound = -0.2;//1.5
...
//if(velocity_circle){
//  ux = 2.5*cos(theta_c/(2*pi));
//  uy = 2.5*sin(theta_c/(2*pi));
//  theta_c += 0.03;
//}
...
if((controller_state == positive_engaged) && FLx_filt < 0.1){
...
if((controller_state == negative_engaged) && FLx_filt > -0.1){
  controller_state = disengaged;
  vir.x = host_mocap.pose.position.x;
  last_updat_time = time_now;
}
...
if((controller_state_y == positive_engaged) && FLy_filt < 0.1){
...
if((controller_state_y == negative_engaged) && FLy_filt > -0.1){
  controller_state_y = disengaged;
  vir.y = host_mocap.pose.position.y;
  last_updat_time = time_now;
}
...
int main(int argc, char **argv)
{
  ...
  //mocap_pos_pub2.publish(host_mocap2);
        ...
        if(velocity_circle){
          vir1.x = 1.0*cos(theta_c / (2*pi))+0.8;
          vir1.y = 1.0*sin(theta_c / (2*pi))+0;
          theta_c += 0.07;
        }
